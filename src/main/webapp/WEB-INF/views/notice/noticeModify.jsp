...
<c:set var="b" value="${noticeModify}" />
			...
			<div class="width-1280 page-margin community-centent">
				<div class="insert-board-center">
					<div class="community-center-size">
						...
						<form id="contentForm" action="${path}/notice/updateNotice"
							method="post">
							<input type="hidden" name="boardNo" value="${b.noticeNo }">
							<select class="insert-select-div" name="boardCategories"
								value="${b.noticeCategories }">
								<option hidden="hidden" value="${b.noticeCategories }">
								<c:choose>
									<c:when test="${b.noticeCategories == 1}">커뮤니티</c:when>
									<c:when test="${b.noticeCategories == 2}">포인트상점</c:when>
									<c:when test="${b.noticeCategories == 3}">플레이</c:when>
								</c:choose>
								</option>
							...
							<div class="content1" >${b.noticeContent }</div>
							<input type="hidden" id="content1" name="content1"
								value='${b.noticeContent }'>
